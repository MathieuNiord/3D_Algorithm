...
let preLoadingImages = new Promise((resolve, reject) => {
	var promises = {};
	for (const subFolder of IMAGES_FOLDERS) {
		var images = [];
		for (const imageName of IMAGES_NAMES) {
			const image = new Image();
			image.src = `${SKYBOX_TEXTURES_URL}${subFolder}/${imageName}.jpg`;
			images.push(image);
		}
		promises[subFolder] = images;
	}
	resolve(promises);
	reject("error");
});
...
class objmesh {
	...
	setShadersParams() {
		...
		this.shader.uIsFrostedMirrorUniform = gl.getUniformLocation(this.shader, "uIsFrostedMirror");
		...
		gl.uniform1i(this.shader.uIsFrostedMirrorUniform, CONTROLLER.isFrostedMirror);
...
// =====================================================
// CUBEMAP, A cube with 6 textures inside
// =====================================================
